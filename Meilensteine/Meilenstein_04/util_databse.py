...
def database_create(database):
    ...
    if not current_tables.get("user"):
        execute_sql="CREATE TABLE `user` ( `user_id` INT NOT NULL AUTO_INCREMENT , `username` VARCHAR(80) NOT NULL , `email` VARCHAR(100) NOT NULL , `password` VARCHAR(64) NOT NULL , `time` TIMESTAMP on update CURRENT_TIMESTAMP NOT NULL , `session_token` INT NOT NULL , `rights` BOOLEAN NOT NULL DEFAULT FALSE , PRIMARY KEY (`user_id`), UNIQUE (`username`)) ENGINE = InnoDB;" 
